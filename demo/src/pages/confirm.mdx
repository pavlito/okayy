...
<Preview
  ...
  code={`import { confirm } from 'okayy';
...
<Preview
  ...
  onConfirm: async () => {
    ...
    // throw an error to keep dialog open
...
<Preview
  label="Try it (5s delay)"
  action={() => confirm({
    title: 'Processing...',
    description: 'You can cancel this at any time.',
    confirmText: 'Start',
    cancelableWhileLoading: true,
    onConfirm: () => new Promise((resolve) => setTimeout(resolve, 5000)),
  })}
  code={`const ok = await confirm({
  title: 'Processing...',
  description: 'You can cancel this at any time.',
...
<Preview
  ...
  action={() => confirm.custom((close) => React.createElement('div', { style: { padding: '1.5rem' } },
    React.createElement('h3', { style: { margin: '0 0 0.5rem', fontWeight: 600, fontSize: '1rem', color: 'var(--okayy-text-color)' } }, 'Custom Dialog'),
    React.createElement('p', { style: { margin: '0 0 1rem', color: 'var(--okayy-text-color)', fontSize: '0.875rem' } }, 'This is fully custom content rendered inside the dialog.'),
    React.createElement('div', { style: { display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' } },
      React.createElement('button', { onClick: () => close(false), style: { padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--okayy-cancel-border)', background: 'var(--okayy-cancel-bg)', color: 'var(--okayy-cancel-text)', cursor: 'pointer' } }, 'Cancel'),
      React.createElement('button', { onClick: () => close(true), style: { padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--okayy-confirm-border)', background: 'var(--okayy-confirm-bg)', color: 'var(--okayy-confirm-text)', cursor: 'pointer' } }, 'Confirm'),
    ),
  ))}
...
<Preview
  ...
  action={() => confirm({
    title: 'Large dialog',
    description: 'This dialog uses the large size preset for more content room.',
    size: 'lg',
  })}
  code={`const ok = await confirm({
  title: 'Large dialog',
  description: 'More content room.',
  size: 'lg',
});`}
/>
...
window.addEventListener('okayy:close', (e) => console.log('closed', e.detail.confirmed));
```
...
const c = await confirm('Step 3');
```

Use `confirm.clearQueue()` to discard all pending dialogs. Each pending promise resolves with `false`.
